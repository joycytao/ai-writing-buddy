      ...
      <div className="mx-auto w-full max-w-4xl rounded-[2.5rem] border border-white/80 bg-white/80 p-8 md:p-10 shadow-[0_24px_60px_rgba(120,74,24,0.12)] backdrop-blur-sm text-left">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h2 className="mt-3 text-4xl md:text-5xl font-black tracking-[-0.04em] text-stone-900">識字高手</h2>
            <p className="mt-2 text-sm font-bold uppercase tracking-[0.2em] text-stone-500">Traditional Chinese sightword mode</p>
          </div>
          <button
            onClick={() => setMenuOpen(true)}
            className="rounded-full border border-stone-200 bg-white p-3 text-stone-700 hover:bg-stone-50"
            aria-label="Open chinese literacy menu"
          >
            <FolderOpen size={18} />
          </button>
        </div>
        ...
        <p className="mt-5 text-lg leading-relaxed text-stone-600">
          {isSessionActive
            ? '請唸出目前的字。你也可以說「i do not know」或「next」跳過。'
            : '開啟資料夾選單來載入雲端詞彙或檢查字詞清單。'}
        </p>
        ...
        <div className="mt-6 rounded-[2rem] border border-stone-200 bg-stone-50 p-6 min-h-52 flex items-center justify-center text-center">
          {isSessionActive ? (
            <div>
              <p className="text-xs uppercase tracking-[0.24em] text-stone-500 font-black">目前字詞</p>
            ...
            <div>
              ...
              <p className="mt-3 text-2xl font-black text-stone-800">{bankWords.length} words loaded</p>
              <p className="mt-2 text-sm text-stone-500">Link and load words, then press Start.</p>
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 z-[80] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="w-full max-w-md rounded-2xl border border-stone-200 bg-white p-5 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-black uppercase tracking-[0.16em] text-stone-700">識字高手 menu</p>
...
export default ChineseLiteracyView;
